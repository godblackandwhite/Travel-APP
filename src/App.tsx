...
export default function App() {
  ...
  const [reservChecked, setReservChecked] = useState<Record<string, boolean>>({});
  const [packChecked, setPackChecked] = useState<Record<string, boolean>>({});
  const [souvenirChecked, setSouvenirChecked] = useState<Record<string, boolean>>({});
  const [visitedSlots, setVisitedSlots] = useState<Record<string, boolean>>({});   // { "cityId|slotTitle": true }
  const [photos, setPhotos] = useState<any[]>([]);               // [{ id, cityId, cityName, flag, dataUrl, review, date, rating }]
  ...
  // ── Styles
  const S = {
    app: { minHeight: "100vh", background: "#0d1117", color: "#e6edf3", fontFamily: "'Trebuchet MS','Segoe UI',sans-serif" } as any,
    header: { background: "#0d1117", borderBottom: "1px solid rgba(201,168,76,0.25)", position: "sticky", top: 0, zIndex: 200 } as any,
    hInner: { maxWidth: "1100px", margin: "0 auto", padding: "10px 16px", display: "flex", alignItems: "center", gap: "12px", flexWrap: "wrap" } as any,
    ...
    badge: (t: any) => ({ display: "inline-block", padding: "3px 10px", borderRadius: "12px", fontSize: "11px", fontWeight: "700" as const, textTransform: "uppercase" as const, letterSpacing: "1px", background: (BUDGET_BADGE as any)[t]?.bg || "rgba(255,255,255,0.1)", color: (BUDGET_BADGE as any)[t]?.color || "#8b949e" }),
